"""
stats2.py -- standalone script that only ever touches assets/stats.svg.

Fetches live GitHub statistics for USERNAME and writes them into
assets/stats.svg. Does not read or write assets/dark.svg or the
original stats_cache.json -- keeps its own cache file
(stats2_cache.json) so it can't collide with status.py.
...
This version prints extra diagnostics (raw profile fields, per-year
commit contribution counts, per-repo LOC results) so a run that still
comes back with all zeros can actually be debugged from the Actions
...
CACHE_FILE = "stats2_cache.json"
SVG_FILE = "assets/stats.svg"
...
def get_profile():
    ...
    if user is None:
        raise RuntimeError(
            ...
            "Check that USERNAME matches your GitHub login exactly "
            "(case-sensitive) and that ACCESS_TOKEN is valid."
        )

    print(
        "  profile debug -> "
        f"login-checked={USERNAME!r}, "
        f"databaseId={user.get('databaseId')}, "
        f"repo totalCount={user.get('repositories', {}).get('totalCount')}, "
        f"followers={user.get('followers', {}).get('totalCount')}, "
...
def get_total_commit_contributions(created_at):
    ...
    for year in range(start_year, current_year + 1):
        ...
        year_total = result["totalCommitContributions"] + result["restrictedContributionsCount"]
        if year_total:
            print(f"  commits debug -> {year}: {year_total}")
        total += year_total
...
def main():
    if not os.path.exists(SVG_FILE):
        raise FileNotFoundError(
            f"{SVG_FILE} not found. Check the path is correct relative to "
            "the repo root (this script only ever touches assets/stats.svg)."
        )

    print(f"Target file: {SVG_FILE}")
    ...
    if all(v == 0 for v in stats.values()):
        print(
            "WARNING: every stat is 0. This means the GraphQL query itself "
            "returned empty/zero data for this user -- check the "
            "'profile debug' line above. Most likely causes: ACCESS_TOKEN "
            "lacks the 'repo' scope, the token is expired, or USERNAME "
            "doesn't exactly match your GitHub login."
        )
